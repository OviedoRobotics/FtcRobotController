...
//      shooterMotor2Amps = shooterMotor1.getCurrent(MILLIAMPS);
    ...
    // Limelight/Pinpoint offsets are also cleared since the values are no longer valid.
    ...
    /*--------------------------------------------------------------------------------------------*/
    // Calculate the offset between limelight and pinpoint positions (doesn't apply it)
    // Only updates X or Y if the new reading has better confidence (lower stddev) than previous
    // X and Y are tracked independently - we might update one but not the other
    // Call this when you have a good limelight reading and want to track the difference
